...
export type Identity = { environment: "operations-backup" | "operations-restore"; eventName: "workflow_dispatch" | "schedule"; run: string; attempt: string; expiresAt: number };
...
export function claimsIdentity(p: JWTPayload, pins: IdentityPins, now: number): Identity {
  ...
  check(p.repository === REPOSITORY && p.repository_owner === "justphilgud" &&
    ...
    p.ref === "refs/heads/main" && p.workflow_ref === WORKFLOW &&
    (p.environment === "operations-backup" || p.environment === "operations-restore"), "IDENTITY_REJECTED");
  check((p.environment === "operations-backup" && (p.event_name === "workflow_dispatch" || p.event_name === "schedule")) ||
    (p.environment === "operations-restore" && p.event_name === "workflow_dispatch"), "IDENTITY_REJECTED");
  ...
  return { environment: p.environment, eventName: p.event_name, run: p.run_id, attempt: p.run_attempt, expiresAt: p.exp * 1000 };
}
