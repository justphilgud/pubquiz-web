...
import { AUDIENCE, check, ISSUER, REPOSITORY, WORKFLOW } from "./contract";
...
export function claimsIdentity(p: JWTPayload, pins: IdentityPins, now: number): Identity {
  ...
  const subjects = [
    `repo:${REPOSITORY}:environment:${p.environment}`,
    `repo:justphilgud@${pins.ownerId}/pubquiz-web@${pins.repositoryId}:environment:${p.environment}`,
  ];
  check(typeof p.sub === "string" && subjects.includes(p.sub) && typeof p.sha === "string" && /^[a-f0-9]{40}$/.test(p.sha) &&
