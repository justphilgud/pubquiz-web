...
export function assertRestoreTarget(value: string | undefined, environment: string, expectedTemporaryHost: string) {
  ...
  assertOperationTransport(new URL(value!));
  return actual;
}

// Neon/libpq contract: TLS AND mandatory SCRAM channel binding. CA verification
// may additionally be requested; plain require without binding is never accepted.
export function assertOperationTransport(url: URL) {
  const keys = [...url.searchParams.keys()];
  requireCondition(!url.hash, "DATABASE_FRAGMENT_REJECTED");
  requireCondition(new Set(keys).size === keys.length, "DATABASE_DUPLICATE_OPTION_REJECTED");
  requireCondition(keys.every(key => ["schema", "sslmode", "channel_binding"].includes(key)), "DATABASE_OPTION_REJECTED");
  requireCondition(["require", "verify-full"].includes(url.searchParams.get("sslmode") ?? ""), "DATABASE_TLS_REQUIRED");
