...
export async function getPublicQuizCalendar(now = new Date()) {
  const today = getBerlinDate(now);
  const series = await prisma.eventreihen.findMany({
    ...
    orderBy: [{ oeffentlicher_name: "asc" }, { eventreihe_id: "asc" }],
    select: {
      oeffentlicher_name: true,
      beschreibung: true,
      ist_oeffentlich: true,
  ...
  return buildPublicQuizCalendar(
    series.map((entry) => ({
      publicName: entry.oeffentlicher_name,
      description: entry.beschreibung,
      isPublic: entry.ist_oeffentlich,
      isArchived: entry.ist_archiviert,
      quizzes: entry.quiz.map((quiz) => ({
        id: quiz.quiz_id,
        title: quiz.titel?.trim() || `Quiz ${quiz.quiz_id}`,
        date: quiz.quiz_datum,
        time: quiz.veranstaltungszeit,
