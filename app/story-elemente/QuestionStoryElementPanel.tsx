...
import { linkQuestionStoryElement, reorderQuestionStoryElements, unlinkQuestionStoryElement, updateQuestionStoryElementPlacement } from "./questionActions";
...
import { storyPlacementFromRelationship } from "./storyPlacement";
...
export default function QuestionStoryElementPanel({ questionId, links, options, canEdit, editorOptions }: { questionId: number; links: StoryLink[]; options: StoryOption[]; canEdit: boolean; editorOptions: StoryElementEditorOptions }) {
  ...
  return <details className="mx-auto mb-10 w-full max-w-4xl rounded-2xl border border-slate-200 bg-white p-4 text-slate-950 sm:p-6">
    ...
    <p className="mt-2 text-sm text-slate-600">Die Standardposition gilt in jedem Quiz, solange dort kein eigener Override gesetzt ist.</p>
    ...
    {canEdit && links.length > 0 && <div className="mt-4 space-y-3 rounded-xl border border-slate-200 bg-slate-50 p-3">{links.map((link) => <label key={link.id} className="block"><span className="mb-1 block text-xs font-bold uppercase tracking-wide text-slate-500">Standardposition für {link.title}</span><select value={storyPlacementFromRelationship(link.relationship)} disabled={pending} onChange={(event) => run(() => updateQuestionStoryElementPlacement({ questionId, storyElementId: link.id, placement: event.target.value as "BEFORE_QUESTION" | "AFTER_SOLUTION" }))} className="min-h-11 w-full rounded-xl border border-slate-300 bg-white px-3 sm:max-w-sm"><option value="BEFORE_QUESTION">Vor der Frage</option><option value="AFTER_SOLUTION">Nach der Auflösung</option></select></label>)}</div>}
