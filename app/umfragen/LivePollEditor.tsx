  ...
  return <div className="space-y-5">
    <section className={panelClass}>
      ...
      {type === "SINGLE_CHOICE" ? <div className="mt-4 space-y-2">
        <div className="flex items-center justify-between"><strong className="text-sm text-slate-800">Optionen (2–6)</strong><button type="button" className="min-h-11 px-2 text-sm font-semibold text-cyan-800 disabled:opacity-40 sm:min-h-0 sm:px-0" disabled={!canEdit || pending || pollOptions.length >= 6} onClick={() => setPollOptions((current) => [...current, newOption(current.length)])}>+ Option</button></div>
