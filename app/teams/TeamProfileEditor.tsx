...
export function TeamProfileEditor({
  ...
  return (
    <section className="answer-team-profile mt-4 rounded-2xl border border-slate-300 bg-white p-3 text-slate-950 shadow-sm sm:p-4" aria-label="Globales Teamprofil">
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="answer-ui-muted text-sm text-slate-600">Euer globales Teamprofil</p>
        </div>
        <button
          type="button"
          ...
          className="answer-ui-control inline-flex min-h-11 min-w-11 items-center justify-center rounded-xl border border-slate-300 bg-white text-slate-700 transition hover:bg-slate-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900"
        >
      ...
      {isOpen && (
        <div id={panelId} className="mt-4 space-y-4 border-t border-slate-200 pt-4">
          <div>
            ...
            <p className="answer-ui-muted mb-2 text-sm text-slate-600">Foto und Avatar gelten in allen Eventreihen.</p>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <button type="button" disabled={pending || profile.photoUploadLocked} onClick={() => cameraRef.current?.click()} className="answer-ui-control min-h-11 rounded-xl border border-slate-500 bg-white px-4 py-2 font-semibold text-slate-950 transition hover:bg-slate-100 active:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 disabled:opacity-100">Foto aufnehmen</button>
            <button type="button" disabled={pending || profile.photoUploadLocked} onClick={() => libraryRef.current?.click()} className="answer-ui-control min-h-11 rounded-xl border border-slate-500 bg-white px-4 py-2 font-semibold text-slate-950 transition hover:bg-slate-100 active:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 disabled:opacity-100">Aus Mediathek wählen</button>
            {profile.photoUrl && <button type="button" disabled={pending} onClick={() => void removePhoto()} className="answer-ui-danger-control min-h-11 rounded-xl border border-red-400 bg-white px-4 py-2 font-semibold text-red-800 transition hover:bg-red-50 active:bg-red-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-800 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400 disabled:opacity-100">Foto entfernen</button>}
            <a href={calendarSubscriptionUrl} className="answer-ui-control inline-flex min-h-11 items-center justify-center rounded-xl border border-slate-500 bg-white px-4 py-2 text-center font-semibold text-slate-900 transition hover:bg-slate-100 active:bg-slate-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 focus-visible:ring-offset-2">PubQuiz-Kalender abonnieren</a>
          </div>
          {profile.photoUploadLocked && <p className="answer-ui-warning rounded-xl bg-amber-50 p-3 text-sm font-semibold text-amber-900">Foto-Uploads sind für dieses Team gesperrt. Ihr könnt weiterhin einen Avatar wählen.</p>}
          {message && (
            <p
              role={message.kind === "ERROR" ? "alert" : "status"}
              aria-live="polite"
              className={message.kind === "ERROR"
                ? "answer-ui-error rounded-xl border border-red-300 bg-red-50 p-3 text-sm font-semibold text-red-800"
                : "answer-ui-success rounded-xl border border-emerald-300 bg-emerald-50 p-3 text-sm font-semibold text-emerald-800"}
            >
              {message.text}
            </p>
          )}
