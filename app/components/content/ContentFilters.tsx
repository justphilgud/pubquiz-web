...
export default function ContentFilters({ filters, categories, eventSeries, templates, loading, onChange, onTemplateChange, onApply, onReset }: {
  ...
  const filterCount = [filters.contentType !== "ALL", filters.templateId !== null, filters.categoryIds.length > 0, filters.storyType !== "ALL", filters.status !== "ALL", filters.questionLifecycle !== "ALL", filters.media !== "ALL", filters.usage !== "ALL", filters.eventSeriesId !== null].filter(Boolean).length;
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <ContentSearchControls query={filters.query} loading={loading} placeholder="Fragen, Story-Elemente und Umfragen durchsuchen …" filterCount={filterCount} filtersOpen={open}
        ...
        <div className="mt-3 grid gap-3 rounded-2xl bg-slate-50 p-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          {(filters.contentType === "QUESTION" || filters.contentType === "ALL") && <label><span className="mb-1 block text-xs font-bold text-slate-600">Template</span><select aria-label="Template" value={filters.templateId ?? ""} onChange={(event) => onTemplateChange(event.target.value || null)} className={inputClass}><option value="">Alle Templates</option>{templates.filter((template) => template.availableForFiltering).map((template) => <option key={template.id} value={template.id}>{template.name}</option>)}</select></label>}
