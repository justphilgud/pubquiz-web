...
async function ensureQuizBlockQuestionItems(
  ...
  const existingStoryIds = new Set(
    existingStoryPlacements.flatMap((placement) =>
      placement.story_element_revision
        ? [placement.story_element_revision.story_element_id]
        : [],
    ),
  );
  ...
  for (const linked of linkedStories) {
    ...
    if (existingStoryIds.has(linked.story_element_id)) continue;
    ...
    await tx.quiz_ablauf_elemente.create({
      data: {
        ...
        story_beziehung: "AFTER_SOLUTION",
        sortierung: order,
        ist_sichtbar: true,
    ...
    existingStoryIds.add(linked.story_element_id);
  }
