...
import {
  filterPresentationTemplates,
  presentationTemplateCardAccent,
  presentationTemplateCardBackground,
} from "@/app/rendering/presentationTemplates/templateOverviewPolicy";
...
export default async function TemplatesPage({ searchParams }: Props) {
  ...
  return <><AppHeader /><main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-900 md:px-8"><div className="mx-auto max-w-7xl space-y-6">
    ...
    <form className="grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:grid-cols-[1fr_180px_180px_auto]">
      ...
      <select name="status" defaultValue={filters.status ?? "CURRENT"} className="min-h-11 rounded-xl border border-slate-300 px-3"><option value="CURRENT">Aktiv & System</option><option value="ALL">Alle Status</option><option value="SYSTEM">System</option><option value="DRAFT">Entwurf</option><option value="ACTIVE">Aktiv</option><option value="ARCHIVED">Archiviert</option></select>
